...
z_bound = 0;          % Fixed-elevation boundary elevation (m)
...
z_max_plot = 250;                 % Maximum elevation used for plotting. Set negative or zero to instead use the max elevation of the initial topo.
cinterval = 5;                  % contour interval for plotting, m
tmax = 10e6;                     % years
plottime = 20000;               % years between plots
tracktime = .1 * plottime;      % years between recording the current erosion rates

% shaded relief plot controls
VE = 5;                        % (times) Vertical exaggeration of plot
lightpos = [10 5 10];
ambient = 0.5;
az = -50;
el = 50;
...
% base level control        
rock_uplift = .5e-4;      % Initial/default rock uplift rate relative to boundary condition (m/yr)
...
k0 = 5e-4;              % fluvial erodibility constant
m0 = 0.5;               % Area exponent for basic stream power
n0 = 1;                 % Shear stress exponent for detachment-limited erosion
kappa0 = 1e-3;          % Hillslope transport coefficient (m/year) - scale rate. This is the depth-averaged velocity of regolith at a slope of 1 (45 degrees) if transport is linear in slope.
sc0 = tand(90);         % Critical slope (L/L) at which transport rate is infinite (Roering)
...
RTproplist = [1 k0/200 0.5  1 1e-4 tand(60)  1.7;          % caprock
              ...
              3 k0     m0  n0 kappa0 sc0  rfslope0;        % soft debris
              4 k0/2   0.5  1 5e-4 tand(30)  inf]';        % medium debris
...
RF_Debris_Rtype = 4;    % Which of the above defined rocktypes represents rockfall debris?
...
bts_dzdy = -0.06;   % dip magnitude (m/m) in y-direction
bts_inty = (rock_uplift*1.8e6) ./ (bts_dzdy * (y/dy));    % surface y-intercept of upper caprock top edge (m)

%% x-tilt not implemented with current rocktype constructor
% bts_dzdx = 0;       % dip magnitude (m/m) in x-direction
% bts_intx = 0;       % surface x-intercept of upper caprock top edge (m)

bts_h1 = 20;        % thickness of upper caprock (m)
bts_h2 = 10;        % thickness of second caprock (m)
bts_sp = 30;        % spacing between bottom of upper and top of lower caprock (m)

%% These next few lines build the stratigraphy array with the y-tilt and layer spacing values specified above. 
backtiltstrat = ones(y,7);backtiltstrat(:,1) = 1;backtiltstrat(:,3) = 1:1:y;backtiltstrat(:,4) = bts_dzdy * dy .* (backtiltstrat(:,3)) + bts_inty;backtiltstrat(:,6) = backtiltstrat(:,3);backtiltstrat(:,5) = backtiltstrat(:,5).*x;backtiltstrat(:,7) = backtiltstrat(:,4)-bts_h1;backtiltstrat(backtiltstrat == 0) = 1;
backtiltstrat2lyr1 = backtiltstrat;
backtiltstrat2lyr2 = backtiltstrat2lyr1;
backtiltstrat2lyr2(:,4) = backtiltstrat2lyr1(:,7) - bts_sp;          % top of lower caprock layer
backtiltstrat2lyr2(:,7) = backtiltstrat2lyr1(:,7) - bts_sp - bts_h2; % bottom of lower caprock layer

% notch the center by setting the middle column rocktype to a weaker type
%notch = [0 x/2 1 z_init+1000 x/2+1 y z_init-2000];
notch1 = ones(y,7);notch1(:,1) = 3;notch1(:,2) = x/2;notch1(:,3) = 1:1:y;notch1(:,4) = bts_dzdy * dy .* (notch1(:,3)) + bts_inty;notch1(:,5) = x/2+1;notch1(:,6) = notch1(:,3);notch1(:,7) = notch1(:,4)-bts_h1;
notch2 = notch1;
notch2(:,4) = notch1(:,7) - bts_sp;
notch2(:,7) = notch1(:,7) - bts_sp - bts_h2;

backtiltstrat2lyr = [backtiltstrat2lyr1; backtiltstrat2lyr2; notch1; notch2];
stratlist = backtiltstrat2lyr';
